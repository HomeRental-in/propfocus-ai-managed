...
| Panel placement                 | Add `PropfocusAI:propfocusLeadLinkGen` to the Lead FlexiPage                                           | On heavily customized pages where the whole layout is **one full-page custom LWC** (e.g. Sumadhura's `verticalNavigationLeadView`), place the Propfocus panel **first** in a single-column region. Placed after, the custom card renders on top of it; a right-sidebar template does not help (the card floats over the sidebar too); and wrapping both in a `flexipage:tabset` broke the customer LWC outright (it mounted its icon rail but never drew its card inside a tab facet). Panel-first keeps the header and all generate buttons visible, and every generate flow opens an SLDS modal above the overlay. The panel's inline history/insights sections stay hidden until the customer's dev constrains their component to its region — that CSS fix is the durable solution and belongs to the customer team. |
